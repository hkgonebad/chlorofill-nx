...
const handleShareItem = ({ title, url, type, itemId }) => {
  if (openShareModal) {
    const shareUrl = type && itemId ? `${window.location.origin}/${type}/${itemId}` : window.location.href;
    openShareModal({
      title,
      url: shareUrl,
      text: type === "meal" ? `Check out this recipe: ${title}` : `Check out this cocktail: ${title}`,
      type,
    });
  } else {
